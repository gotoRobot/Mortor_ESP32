...
void app_main(void)
{
    ...
    xTaskCreate(Position_thread, "Position_thread TickType_t xLastWakeTime;
    xLastWakeTime = xTaskGetTickCount();", 4095, NULL, 4, NULL);
}
...
void Position_thread(void *arg)
{
    ...
    while (1)
    {
        p_current=g_encoder->get_counter_value(g_encoder);//读取
        if(p_current>SINGLE_STEP){   
            int8_t cnt=p_current/SINGLE_STEP;
            counter+=cnt;
            p_current=p_current%SINGLE_STEP;
        }
        if(p_current>HALF_STEP){
            p_error=-p_current;
            i_target=MAX_I*(float)(p_error/HALF_STEP);
        }
        else{
            p_error=p_current;
            i_target=MAX_I*(float)(p_error/HALF_STEP);
